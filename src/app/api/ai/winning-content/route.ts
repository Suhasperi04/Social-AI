...
import { buildWinningContentPrompt } from "@/lib/ai/prompts";
import { checkUsage, incrementUsage } from "@/lib/usage";
import { checkRateLimit } from "@/lib/rate-limit";
import { WinningContentData } from "@/types";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimit = await checkRateLimit(instagramId, "winning_content");
    ...
    if (!body?.forceRefresh) {
      const { data: cached } = await supabase
        .from("reports").select("*")
        .eq("instagram_id", instagramId).eq("report_type", "winning_content")
    ...
    const mediaWithInsights = await igApi.getMediaWithInsights(50);

    const prompt = buildWinningContentPrompt(mediaWithInsights);
    const analysis = await generateAnalysis<WinningContentData>(prompt);

    await supabase.from("reports").insert({ instagram_id: instagramId, report_type: "winning_content", data: analysis });
    await incrementUsage(instagramId, "reports");

    return NextResponse.json({ data: analysis, cached: false });
  } catch (error) {
    console.error("Winning content error:", error);
    return NextResponse.json({ error: "Failed to generate winning content report" }, { status: 500 });
  }
}
