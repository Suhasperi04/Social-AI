...
import { buildAccountHealthPrompt } from "@/lib/ai/prompts";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Rate limit check
    const rateLimit = await checkRateLimit(instagramId, "account_health");
    ...
    if (!usage.allowed) {
      return NextResponse.json(
        { error: "Report limit reached", usage },
        { status: 403 }
      );
    }
    ...
    // Get account & fetch Instagram data
    ...
    const [profile, mediaWithInsights] = await Promise.all([
      igApi.getProfile(),
      igApi.getMediaWithInsights(30),
    ]);

    // Generate AI analysis
    const prompt = buildAccountHealthPrompt(profile, mediaWithInsights);
    const analysis = await generateAnalysis<AccountHealthData>(prompt);
    ...
    return NextResponse.json(
      { error: "Failed to generate account health report" },
      { status: 500 }
    );
  }
}
