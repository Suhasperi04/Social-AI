...
import { buildBestTimePrompt } from "@/lib/ai/prompts";
import { checkUsage, incrementUsage } from "@/lib/usage";
import { checkRateLimit } from "@/lib/rate-limit";
import { BestTimeData } from "@/types";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimit = await checkRateLimit(instagramId, "best_time");
    ...
    if (!body?.forceRefresh) {
      const { data: cached } = await supabase
        .from("reports").select("*")
        .eq("instagram_id", instagramId).eq("report_type", "best_time")
    ...
    const prompt = buildBestTimePrompt(mediaWithInsights);
    const analysis = await generateAnalysis<BestTimeData>(prompt);

    await supabase.from("reports").insert({ instagram_id: instagramId, report_type: "best_time", data: analysis });
    ...
    return NextResponse.json({ error: "Failed to analyze best posting times" }, { status: 500 });
  }
}
